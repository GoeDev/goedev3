...
class allign2{
	...
	public static void globalallign(int g, int[][] matrix){
		...
		for(int i = 0; i<seq1.length(); i++){
			for (int j = 0; j<seq2.length(); j++){
				...
				if(i != 0 && j != 0){
					max = Maximum(array[i][j-1].getMax()-g, array[i-1][j-1].getMax() + getWeight(matrix, i, j), array[i-1][j].getMax()-g);//s fehlt
